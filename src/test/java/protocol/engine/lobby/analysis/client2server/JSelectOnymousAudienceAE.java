...
public class JSelectOnymousAudienceAE implements SelectOnymousAudienceAnalysisEngine {
    @Override
    public Try<LobbyMessageProtocol> process(LobbyBOX box, SelectOnymousAudienceProtocol selectOnymousAudienceProtocol) {
        if (box instanceof JLobbyBox) {
            return Success.apply(LobbyMessageTestProtocol.apply(new SelectOnymousAudience(null).type()));
        } else {
