...
public class JSelectHumanPlayerAE implements SelectHumanPlayerAnalysisEngine {
    @Override
    public Try<LobbyMessageProtocol> process(LobbyBOX box, SelectHumanPlayerProtocol selectHumanPlayerProtocol) {
        if (box instanceof JLobbyBox) {
            return Success.apply(LobbyMessageTestProtocol.apply(new SelectHumanPlayer(null).type()));
        } else {
