...
import licos.protocol.engine.analysis.village.client2server.server2logger.OnymousAudienceBoardAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object OnymousAudienceBoardProcessingEngine {
  ...
  def process(
      ...
      engine:          OnymousAudienceBoardAnalysisEngine,
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            OnymousAudienceBoardProtocol.read(json, village) match {
              case Some(protocol) =>
          ...
          case Failure(_: Throwable) =>
            Failure(new VillageCreationException(OnymousAudienceBoardAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(OnymousAudienceBoardAnalysisEngine.name))
    }
  }
}
