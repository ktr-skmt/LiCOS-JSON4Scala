...
import licos.protocol.engine.analysis.village.server2client.server2logger.FlavorTextAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object FlavorTextProcessingEngine {
  ...
  def process(
      ...
      engine:          FlavorTextAnalysisEngine,
      json:            JsonFlavorText
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            FlavorTextProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(FlavorTextAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(FlavorTextAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(FlavorTextAnalysisEngine.name))
    }
  }
}
