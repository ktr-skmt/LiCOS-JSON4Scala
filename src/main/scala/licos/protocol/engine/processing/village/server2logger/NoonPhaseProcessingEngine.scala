...
import licos.protocol.engine.analysis.village.server2client.server2logger.NoonPhaseAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object NoonPhaseProcessingEngine {
  ...
  def process(
      ...
      engine:          NoonPhaseAnalysisEngine,
      json:            JsonPhase
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            NoonPhaseProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(NoonPhaseAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(NoonPhaseAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(NoonPhaseAnalysisEngine.name))
    }
  }
}
