...
import licos.protocol.engine.analysis.village.client2server.server2logger.ErrorFromClientAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object ErrorFromClientProcessingEngine {
  ...
  def process(
      ...
      engine:          ErrorFromClientAnalysisEngine,
      json:            JsonError
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            ErrorFromClientProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(ErrorFromClientAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(ErrorFromClientAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(ErrorFromClientAnalysisEngine.name))
    }
  }
}
