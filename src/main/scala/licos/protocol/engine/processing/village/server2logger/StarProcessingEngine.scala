...
import licos.protocol.engine.analysis.village.client2server.server2logger.StarAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object StarProcessingEngine {
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            StarProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(StarAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(StarAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(StarAnalysisEngine.name))
    }
  }
}
