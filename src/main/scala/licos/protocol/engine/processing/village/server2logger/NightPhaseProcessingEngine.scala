...
import licos.protocol.engine.analysis.village.server2client.server2logger.NightPhaseAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object NightPhaseProcessingEngine {
  ...
  def process(
      ...
      engine:          NightPhaseAnalysisEngine,
      json:            JsonPhase
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            NightPhaseProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(NightPhaseAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(NightPhaseAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(NightPhaseAnalysisEngine.name))
    }
  }
}
