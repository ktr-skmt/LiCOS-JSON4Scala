...
import licos.protocol.engine.analysis.village.server2client.server2logger.GameResultAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object GameResultProcessingEngine {
  ...
  def process(
      ...
      engine:          GameResultAnalysisEngine,
      json:            JsonGameResult
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            GameResultProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(GameResultAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(GameResultAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(GameResultAnalysisEngine.name))
    }
  }
}
