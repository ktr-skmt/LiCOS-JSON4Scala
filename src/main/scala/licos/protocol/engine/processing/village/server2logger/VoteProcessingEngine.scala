      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            VoteProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(VoteAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(VoteAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(VoteAnalysisEngine.name))
    }
  }
}
