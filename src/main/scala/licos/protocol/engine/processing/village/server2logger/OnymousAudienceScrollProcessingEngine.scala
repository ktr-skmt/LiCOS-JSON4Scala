...
import licos.protocol.engine.analysis.village.client2server.server2logger.OnymousAudienceScrollAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object OnymousAudienceScrollProcessingEngine {
  ...
  def process(
      ...
      engine:          OnymousAudienceScrollAnalysisEngine,
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            OnymousAudienceScrollProtocol.read(json, village) match {
              case Some(protocol) =>
          ...
          case Failure(_: Throwable) =>
            Failure(new VillageCreationException(OnymousAudienceScrollAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(OnymousAudienceScrollAnalysisEngine.name))
    }
  }
}
