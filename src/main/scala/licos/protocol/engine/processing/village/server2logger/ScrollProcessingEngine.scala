...
import licos.protocol.engine.analysis.village.client2server.server2logger.ScrollAnalysisEngine
import licos.protocol.engine.processing.{JSON2ProtocolException, VillageBOX}

import scala.concurrent.duration._
import scala.util.{Failure, Success, Try}

object ScrollProcessingEngine {
      ...
      case Some(phase: Phase) =>
        villageFactory
          ...
          case Success(village: Village) =>
            ScrollProtocol.read(json, village) match {
              case Some(protocol) =>
                engine.process(box, protocol)
              case None => Failure(new JSON2ProtocolException(ScrollAnalysisEngine.name))
            }
          case Failure(_: Throwable) => Failure(new VillageCreationException(ScrollAnalysisEngine.name))
        }
      case None => Failure(new PhaseException(ScrollAnalysisEngine.name))
    }
  }
}
