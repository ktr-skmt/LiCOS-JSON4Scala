...
final case class NightPhaseProtocol(state: VillageState, character: Seq[CharacterProtocol], role: Seq[RoleProtocol]) {

  val json: Option[JsonPhase] = {
    server2logger.NightPhaseProtocol(state, character, role, Nil).json
  }
