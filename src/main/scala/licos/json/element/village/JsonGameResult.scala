...
case class JsonGameResult private (base: JsonBase,
  ...
  def this(base: JsonBase,
           character: Seq[JsonResultCharacter],
           role: Seq[JsonResultRole]) = {
    this(
      base: JsonBase,
      JsonSubGameResult(
        character.sortWith { (a1: JsonResultCharacter, a2: JsonResultCharacter) => a1.name.en < a2.name.en }.sortBy(!_.isMine): Seq[JsonResultCharacter],
  ...
  def this(base: JsonBase,
           character: JList[JsonResultCharacter],
           role: JList[JsonResultRole]) = {
    this(base: JsonBase,
         character.asScala.sortWith { (a1: JsonResultCharacter, a2: JsonResultCharacter) => a1.name.en < a2.name.en }.sortBy(!_.isMine): Seq[JsonResultCharacter],
         role.asScala.sortWith { (r1: JsonResultRole, r2: JsonResultRole) => r1.name.en < r2.name.en }: Seq[JsonResultRole])
  }

  def character: Seq[JsonResultCharacter] = sub.character.sortWith { (a1: JsonResultCharacter, a2: JsonResultCharacter) => a1.name.en < a2.name.en }.sortBy(!_.isMine)
...
case class JsonSubGameResult(character: Seq[JsonResultCharacter],
                             role: Seq[JsonResultRole])
